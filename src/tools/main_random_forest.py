...
from tools_random_forest import hyper_params_search, hyper_params_search_with_feature_elimination
...
if save_choice.lower() == 'y':
    if choice.lower() == 'p':
        ...
        with open(r"..\tools\producer\random_forest_prod.pickle", 'wb') as content:
            pickle.dump(king_model, content)
        # Recording of the list of variables used for the optimal model
        with open(r"..\tools\producer\features_random_forest_prod.txt", 'w', encoding='utf-8') as l:
            for line in best_features:
                l.write(f"{line}\n")
                print("Producer model successfully saved!")
    else:
        # Optimal model (consumer case) model in a pickle
        with open(r"..\tools\consumer\random_forest_conso.pickle", 'wb') as content:
            pickle.dump(king_model, content)
        # Recording of the list of variables used for the optimal model
        with open(r"..\tools\consumer\features_random_forest_conso.txt", 'w', encoding='utf-8') as l:
            for line in best_features:
                l.write(f"{line}\n")
